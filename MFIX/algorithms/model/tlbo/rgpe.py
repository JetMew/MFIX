...
class RGPE(BaseTLSurrogate):
    def __init__(self, config_space, source_hpo_data, seed,
                 ...
                 no_descent=True, decay=True, decay_rate=1.02):
        super().__init__(config_space, source_hpo_data, seed,
                         surrogate_type=surrogate_type, num_src_hpo_trial=num_src_hpo_trial)
        self.method_id = 'rgpe'
        self.no_descent = no_descent
        self.decay = decay
        if self.decay:
    ...
    def train(self, X: np.ndarray, y: np.array):
        ...
        self.logger.info('MISRANK: {}'.format(np.mean(ranking_loss_caches, axis=0)/len(y)/len(y)))
        ...
        # non-descent weights.
        if self.no_descent:
            last_w = self.hist_ws[-1]
            if self.w[self.K] < last_w[self.K]:
                self.logger.info('Non-descent target weights')
                for id in range(self.K):
                    self.w[id] = self.w[id] * (1 - last_w[self.K]) / (1 - self.w[self.K])
                self.w[self.K] = last_w[self.K]

        if self.decay and self.iteration_id % 5 == 0:
            for id in range(self.K):
                self.w[id] /= self.decay_rate

            self.w[self.K] = 1 - np.sum([self.w[id] for id in range(self.K)])
            self.logger.info('Low-fidelity weights decay.')
